...
namespace IERM.DAL
{
    public class EccmMaintenanceOrderImplementDAL
    {
        /// <summary>
        /// 根据order_id 获取实施记录
        /// </summary>" ", 
        ...
        {
            int order_device_standard_type = 2;
            StringBuilder sqlCount = new StringBuilder();
            sqlCount.Append(" select count(1) ");
            sqlCount.Append(" from eccm_maintenance_order_implement i ");
            ...
            sqlCount.Append(" left join eccm_maintenance_order o on o.order_id = i.order_id ");
            sqlCount.Append(" left join eccm_order_device_standard s on o.order_sn = s.order_sn and s.equCode = i.equCode and order_device_standard_type = " + order_device_standard_type);
            ...
            sql.Append(" from eccm_maintenance_order_implement i ");
            sql.Append(" left join equipmentinfo e on e.equCode = i.equCode ");
            sql.Append(" left join userinfo u on u.uid = i.uid_handle ");
            sql.Append(" left join eccm_maintenance_order o on o.order_id = i.order_id ");
            sql.Append(" left join eccm_order_device_standard s on o.order_sn = s.order_sn and s.equCode = i.equCode and order_device_standard_type = " + order_device_standard_type);
            sql.AppendFormat(" where i.order_id={0}", orderID);
            sql.AppendFormat(" limit {0},{1} ", (pageindex - 1) * pagesize, pagesize);


            return MySQLHelper.ExecuteToList<EccmMaintenanceOrderImplementModel>(sql.ToString(), null);
        }
        ...
        public int Add(EccmMaintenanceOrderImplementModel model)
        {
            ...
            strSql.Append("insert into eccm_maintenance_order_implement(");
