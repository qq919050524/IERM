...
namespace IERM.Message
{
    public static class YunRongSDK
    {
        ...
        private static string Pwd
        {
            ...
            {
                //解密密码
                return MD5Encrypt.Decrypt(passWord, "ierm");
            }
        }
        ...
        /// <returns></returns>
        public static SMSReceiveModel SendSMS(string phone, string body)
        {
        ...
        private static SMSReceiveModel GetPostReturn(string postReturn)
        {
            ...
            {
                case "0":
                    ResultModel = new SMSReceiveModel { statusCode= postReturn, statusMsg="发送成功"  };
                    break;
                case "-1":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "用户名或口令错误" };
                  
                    break;
                case "-2":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "IP验证错误" };
                    break;
                case "-3":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "定时日期错误" };
                  
                    break;
                case "-10":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "余额不足" };
                  
                    break;
                case "-101":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "userId为空" };
                  
                    break;
                case "-102":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "目标号码为空" };
                   
                    break;
                case "-103":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "内容为空" };
                    
                    break;
                case "200":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "目标号码错误" };
                    break;
                case "201":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "目标号码在黑名单中" };
                    break;
                case "202":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "内容包含敏感单词" };
                    break;
                case "203":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "特服号未分配" };
                    break;
                case "204":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "分配通道错误" };
                    break;
                case "999":
                    ResultModel = new SMSReceiveModel { statusCode = postReturn, statusMsg = "发送三次都超时" };
                    break;
            }
            return ResultModel;
        }
        ...
        private static string sendMessage(string strPhone, string strContent)
        {
            ...
            sb.AppendFormat("&{0}={1}", "body", SignName+" " + strContent); //短信内容	短信内容；短信内容长度+短信签名长度不要超过500;超过500字符系统会自动分割成多条发送。
        ...
        private static string sendBatchMessage(string strPhone, string strContent)
        {
            ...
            sb.AppendFormat("&{0}={1}", "userName", userName);
            sb.AppendFormat("&{0}={1}", "passWord", Pwd);
            sb.AppendFormat("&{0}={1}", "messageId", DateTime.Now.ToString("YYYYmmddHHmmss"));  //应用系统消息ID 如需要状态报告则不能为空，最长50位
            ...
            sb.AppendFormat("&{0}={1}", "body", SignName + " " + strContent);        //短信内容
